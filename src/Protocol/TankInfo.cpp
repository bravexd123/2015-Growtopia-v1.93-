...
bool TankInfo::Serialize(TextParse parser) {
    ...
    this->SetTankIDName(parser.Get("tankIDName", 1));
    this->SetTankIDPass(parser.Get("tankIDPass", 1));

    this->SetRelativeId(parser.Get("rid", 1));

    auto gameVersionStr = parser.Get("game_version", 1);
    if (gameVersionStr.empty())
        return false;
    try {
        this->m_gameVersion = std::stof(gameVersionStr);
    }
    ...
    if (platformID.empty())
        return false;
    try {
        this->m_platformID = std::stoi(platformID);
    }
    catch (const std::exception&) {
        return false;
    }
